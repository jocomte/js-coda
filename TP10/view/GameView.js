...
export default class GameView {
  ...
  render() {
    ...
    if (this.game.players) {
      ...
      // Nettoyer le cache des spritesheets qui ne sont plus utilisés
      this.cleanSpriteCache(playersList);
  ...
  /**
   * Nettoie le spriteCache en supprimant les sprites des joueurs qui ont disparu
   */
  cleanSpriteCache(activePlayers) {
    // Récupérer les skins actuellement utilisés
    const activeSkins = new Set(activePlayers.map((player) => player.skinPath));

    console.log("Skins actifs:", Array.from(activeSkins));
    console.log("Cache avant nettoyage:", Object.keys(this.spriteCache));

    // Supprimer les sprites du cache qui ne sont plus utilisés
    let removed = 0;
    for (const skinPath in this.spriteCache) {
      if (!activeSkins.has(skinPath)) {
        console.log("Suppression du cache:", skinPath);
        delete this.spriteCache[skinPath];
        removed++;
      }
    }

    console.log(`Cache nettoyé: ${removed} sprites supprimés`);
    console.log("Cache après nettoyage:", Object.keys(this.spriteCache));
  }
